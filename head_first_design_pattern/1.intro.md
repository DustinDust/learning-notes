...
- The `SimUDuck` game which has ducks that can quack has a new feature that about to be introduced into the game, which is flying ducks. The book highlight the approach: a parent class `Duck` that has method `fly()` and `quack()`. \
...
- What about interfaces? `Flyable` and `Quackable` interfaces can help separate which duck can fly and which can quack.\
  => Still, overriding methods with 48 types of ducks introduces lots of code duplication. In other words, **NO** code reuse. It solves the part of the problem and create another.
...
- Since the quack's and fly's can be assign, set and modified in during runtime, it helps a lot with FLEXIBILITY
