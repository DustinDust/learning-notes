# Summary

- This video talk about how remote work make trust harder to build, cause manager cant "see" you working anymore.
  => Employee monitoring software (tattleware) usage went up 3-4x during early pandemic, but author argue it's a bad way to measure real work, since people can just wiggle mouse to trick the system.
- Instead, focus should be on results, not activity or "busyness". She give few concrete way to build trust through competency:
  - Grow expertise continuously, pick 1 skill per quarter/year and make a plan to improve it.
  - Keep 1:1 ratio between say and do, if you promise something (even casual one like "I'll get back to you") - actually follow up, put it on calendar so you dont forget.
  - Set results-focused goal, not activity-based one. eg: for recruiter, "hours worked" is activity, but "hire qualified candidate" is result. break it into milestone like: post to 20 job board, get 10 resume, interview 5 people.
  - Promote your own work a little, she share embarrassing story about missing "elevator pitch" moment with her Dean, after that she start keeping a "weekly brag bite" ready to share.
  - Over communicate, specially in remote setting. small update like "on it" or "grading's done" go a long way, compare to someone who just silently does the work (even if equally competent, they earn less trust because manager cant tell whats happening).
  - Touch base with a cadence that fit your work type, some team need daily/weekly check in, other (like project based work) only need touch point at milestone.

# Personal Notes

- key idea: trust from competency = being reliable (do what you say) + communicating that reliability outward. Monitoring ≠ trust, it can actually erode it
- "results focused" goal-setting sounds like core theme, so should always ask "what does success look like"
- Ssmall consistent communication (status update) matter more than i thought, its not about micromanaging yourself just closing the loop.
- Brag Bite concept - useful, i tend to underplay my own work too, might try keep a running list of small wins weekly.
- cadence should match nature of work, not one-size-fit-all (daily standup doesnt make sense for every team)
